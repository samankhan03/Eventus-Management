                ...
                <div class="flex justify-between">
                    ...
                    <div class="relative">
                        <button type="button" class="focus:outline-none" @click="openMenu = !openMenu">
                            <icon name="ellipsis-h" class="w-8 h-8 text-black"></icon>
                        </button>
                        <div v-if="openMenu" class="bg-gray-300 text-sm absolute w-48 right-0 text-gray-700 shadow-lg rounded-md px-4 py-2 hover:bg-gray-700 hover:text-gray-300 transition duration-150 ease-in-out">
                            <form @submit.prevent="deletePost">
                                <button type="submit" class="flex justify-between items-center w-full focus:outline-none">
                                    Delete Post
                                    <icon name="ellipsis-h" class="w-8 h-8 fill-current text-black"></icon>
                                </button>
                            </form>
                        </div>
                    </div>
                </div>
                <p class="bg-gray-100 rounded mt-3 px-3 py-2 text-black">{{ post.body }}</p>
            </div>

            <div class="flex items-end my-3">
                <div>
                    <span class="text-sm italic text-black">{{ post.timeAgo }}</span>
                </div>

                <div class="flex ml-3">
                    <like :item="post" :method="submitLike"></like>
                    <dislike :item="post" :method="submitDislike" class="ml-2 text-black"></dislike>
                </div>
            </div>

            <post-form :method="submit" :form="form" :text="'Comment'" class="text-black"></post-form>

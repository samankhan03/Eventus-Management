    ...
    public function rooms()
{
    ...
    // Example method to join a room (attaching user to a room)
    public function joinRoom(Room $room)
    {
        if (!$this->rooms->contains($room)) {
            $this->rooms()->attach($room);
            $room->increment('active');
        }
    }

    // Example method to leave a room (detaching user from a room)
    public function leaveRoom(Room $room)
    {
        if ($this->rooms->contains($room)) {
            $this->rooms()->detach($room);
            $room->decrement('active');
        }
    }
}
